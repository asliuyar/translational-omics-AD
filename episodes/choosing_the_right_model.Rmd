...
```{r}
NS.INTENSITY <- read.csv(paste0(datadir,"Nanostringdata_logNormalized_BatchCorrected_MergedTg_LOAD1_PrimaryScreenPaper.csv"),check.names = F,row.names=1)
```
...
```{r}
metadata <- read.csv(paste0(datadir,"metadata_NSdata.csv"),check.names = F)
```
...
```{r}
metadata <- metadata %>% dplyr::select(Sample, Sex=SEX, Age=AGE,Genotype,BD=BindingDensity)
```
...
```{r}
NS.INTENSITY.long <- NS.INTENSITY %>% tibble::rownames_to_column(.,"symbol") %>% pivot_longer(cols=-symbol,names_to="Sample",values_to="value")
```
...
```{r}
mydat_with_metadata <- NS.INTENSITY.long %>% left_join(metadata,by="Sample")
```
...
```{r}
effects <- as.data.frame(bind_rows( lapply(lm.results, function(x) x$coefficients[,"Estimate"] ))) %>% mutate(names = names(lm.results)) %>% column_to_rownames(.,var="names")
```
...
```{r}
colnames(effects) <- c("(Intercept)", "Sex (Male)", "BD", "LOAD1","5xFAD",
                                          "Mthfr*677C>T", "Abca7*A1527G", "Ceacam1 KO", "Snx1*D465N", "Clasp2*L163P", 
                                          "Plcg2*M28L","Sorl1*A528T" ,"Mtmr4*V297G","Shc2*V433M","Meox2 KO (HET)","Slc6a17*P61P")
```
...
```{r}
ns_effects <- effects %>% tibble::rownames_to_column(.,"symbol") %>%   dplyr::select(-"(Intercept)",-"BD") %>% 
  pivot_longer(cols=-symbol,names_to="Variant",values_to="value")
```
...
```{r}
ns_vs_ampad_fc <- ns_effects  %>% inner_join(ampad_modules_fc, by = c("symbol"))
```
...
```{r }
ns_effects <- effects %>% tibble::rownames_to_column(.,"Gene") %>% dplyr::select(-"(Intercept)",-"BD") %>% pivot_longer(cols=-Gene,names_to="Variant",values_to="value")
```
