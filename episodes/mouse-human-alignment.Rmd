...
The 'omics comparisons allow for very rich comparisons because a significant proportion of genes are shared between these two species. Additionally, homology at the anatomical and neuropathological levels is less clear.  

![Aligning Human and Mouse Phenotypes](../fig/rmd04-dimensions_of_model_alignment.png){alt='image showing humans and mice along with neuroimaging, neuropathology, and heatmap images representing the different modalities to align.'}
...
![5xFAD Mouse Phenotypes](../fig/5xFAD_phenotype.png){alt='list of AD phenotypes associated with 5xFAD mouse model'}
...
```{r}
# RNA-seq counts
counts <- synapser::synGet('syn22108847') %>% .$path %>% read_tsv()

# biospecimen  metadata
meta <- synapser::synGet('syn22103213') %>% .$path %>% read_csv() %>% 
  select(individualID, specimenID) 

# individual metadata, joined to the above
meta <- synapser::synGet('syn22103212') %>% .$path %>% read_csv() %>% 
...
Ok, there are a total of 423 significantly differentially expressed genes at 4 months of age when comparing 5xFAD to WT brain tissue. The vast majority of these (391) are expressed at higher levels in 5xFAD mice relative to WT. Let's take a look at the most significantly DE genes by ordering the results table by the adjusted p-value:
```{r}
...
fad.deg = tibble(
  ...
  ungroup() %>% 
  mutate(
    dds = map(dds, ~ .x[rowSums(counts(.x)) >= 10,]),
...
map_dbl(fad.deg$res.t, 
  ...
  pull(Ensembl_gene_id) %>% length)
```
There are between 391 and 1873 up-regulated genes, and between 32 and 1863 down-regulated genes.
...
It looks like there are 21, 21, and 414 overlapping down-regulated genes, respectively.
...
```{r}
saveRDS(fad.deg, here::here("results","5xFAD_DESeq_analysis.rds"))
```
...
![Transcriptomic studies of Alzheimer's Disease](../fig/rmd04-ampad_studies_regions.png){alt='table showing AMP-AD studies, the number of samples, and brain regions sampled'}
...
![AMP-AD GENE Modules](../fig/rmd04-consensus_clusters.png){alt='Heatmap showing the overlap between co-expression clusters and the consensus clusters.'}
...
query <- synapser::synTableQuery("SELECT * FROM syn11932957")
...
![Human-mouse logFCcorrelation](../fig/rmd04-hs_mm_logfc_correlation.png){alt='Example correlation plots for AMP-AD co-expression module genes and mouse model expression.'}
...
```{r FAD}
fad.deg <- readRDS( here::here("results","5xFAD_DESeq_analysis.rds") ) %>% 
  mutate(age = str_c(age,'m')) %>% 
  select(age, res.t) %>% 
  unnest(res.t)
...
```{r}
load(here::here("results","AMPAD_Module_Data.RData"))
```
...
model_vs_ampad <- inner_join(fad.deg, ampad_modules_fc, by = c("symbol"), multiple = "all") 
```
...
![Functional enrichment approaches](../fig/rmd04-Zhao_Rhee_TrendsGenetics_pathway_enrichment.jpg){alt='Figure showing different methods to test functional enrichment.'}
...
We'll start by considering the genes that are significantly DE in 6 month old animals
```{r}
...
```{r warning=FALSE, message=FALSE}
gse.enr <- gseGO(gene.list, ont = 'all', OrgDb = org.Mm.eg.db, keyType = 'SYMBOL')
```

How many significant terms are identified:  

```{r}
gse.enr@result %>% filter(p.adjust <= 0.05) %>% pull(ID) %>% unique() %>% length()
```
...
This plot shows the top 15 GSEA enriched terms. The displayed terms are all from the up-regulated class (i.e. `NES > 0`), and consist of many immune-relevant terms (e.g. "immune response" and "leukocyte mediated immunity"). 
...
fad.enr <- fad.deg %>% 
  ...
  ungroup() %>% 
  mutate(
    gse = map(gl, ~gseGO(.x, ont = 'all', org.Mm.eg.db, keyType = 'SYMBOL')),
    res = map(gse, ~.x@result)
  )

saveRDS(fad.enr, here::here('results','5xFAD_gsea_results.rds'))
```
...
![Common pitfalls](../fig/rmd04-Wijesooriya_PLOS_CompBio_F2_cropped.png){alt='Figure from the paper showing the number of papers examined that failed to meet certain standards for reporting the results of functional enrichment testing.'}
...
![Endophenotypes of AD](../fig/rmd04-AD_endophenotypes.png){alt='An image of a brain surrounded by different endophenotype categories that are associated with AD.'}
...
![Common AD Research Ontology](../fig/rmd04-CADRO_2023.jpg){alt='Circle plot showing therapeutics in various stages of clinical trials coded by the CADRO target class.'}

![Drugs in Pipeline by CADRO](../fig/rmd04-CADRO_2023_2.jpg){alt='Bar plot showing the numbers of agents in clinical trials across each class.'}
...
![AD Biodomain Demographics](../fig/rmd04-score_domains_preprint_F1_cropped.jpeg){alt='Heatmaps showing the fraction of overlapping GO terms and genes between the various biological domains.'}

Since their instantiation, the ESSJ TREAT-AD center has grouped the terms within each biodomain into functionally coherent sub-domains. The sub-domains are driven by gene co-annotation and disease risk score enrichment. 

![AD Subdomains](../fig/Fig_subdom_annotation copy.png){alt='Plot showing the number of subdomains associated with each domain and how many GO terms are assigned to each domain.'}
...
Not all of the enriched terms are annotated to a biological domain. Some terms are too broad and not specific (e.g. 'response to other organism'), while others may not have been captured by a biological domain annotation yet (e.g. 'regulation of immune system process'). Remember that the conception of the biodomains involved a requirement that they be modifiable, and these terms may be added to the biodomain definintions in the future. Let's modify the `Biodomain` value for terms that aren't annotated to a domain to 'none'.  
...
bd.tally = tibble(domain = c(unique(biodom$Biodomain), 'no domain')) %>% 
  ...
  mutate(
    ...
    n_sig_term = gse.enr.bd$ID[ gse.enr.bd$Biodomain == domain ] %>% unique() %>% length()
    )
arrange(bd.tally, desc(n_sig_term))
```

Many enriched terms don't map to a domain (`r bd.tally %>% filter(domain == 'no domain') %>% pull(n_sig_term) %>% sum()`), but most do (`r bd.tally %>% filter(domain != 'no domain') %>% pull(n_sig_term) %>% sum()`). Of those that do, the vast majority map into the `Immune Response` biodomain.  

We can plot the enrichment results, stratified by biodomain:  

```{r biodom_plot, fig.width=7, fig.height=5, fig.show='hide', warning=F}
...
This makes it really clear that in the 6 month old 5xFAD mice, the enriched `Immune Response` domain terms are all associated with up-regulated genes, while the enriched `Synapse` domain terms are almost all associated with down-regulated genes. It also highlights the several other domains with significantly enriched terms (e.g. `Structural Stabilization`, `Oxidative Stress`, `Endolysosome`, etc). 

Now let's look across age groups to see how domain enrichments change:
```{r biodom_plot, warning=F, fig.width=12, fig.height=4, fig.show='hide'}
enr.bd_plot <- fad.enr %>% select(age, res) %>% unnest(res) %>% 
  full_join(., biodom %>% select(ID = GO_ID, Biodomain, Subdomain)) %>% 
  ...
  mutate(n_sig = length(unique(ID)), .by = Biodomain) %>% 
...
The earliest transcriptomic changes in the 5xFAD animals are associated with `Immune Response` domain terms. By 6 months, there are enrichments for several other domains (as above), and by 10 months there are significant enrichments in the `Vasculature` and `Mitochondrial Metabolism` domains. 
...
```r
stg_blue.ora <- ampad_modules %>% filter(module == 'STGblue') %>% pull(Mouse_gene_symbol) %>% enrichGO(., org.Mm.eg.db, keyType = 'SYMBOL')
...
hs.gsea <- ampad_modules_raw %>% 
  ...
  ungroup() %>% 
  mutate(
    gse = map(gl, ~gseGO(.x, ont = 'ALL', org.Hs.eg.db::org.Hs.eg.db, keyType = 'SYMBOL')),
    res = map(gse, ~.x@result)
  )
```

Now we can map the biodomains onto the enriched GO terms, and plot the results

```{r biodom_plot_hs, fig.width=7, fig.height=5, fig.show='hide'}
enr.bd_plot <- hs.gsea %>% select(res) %>% unnest(res) %>% 
...
comb.gsea <- fad.enr %>% mutate(model = str_c('5xFAD, ',age)) %>% select(model, res) %>% unnest(res) %>% 
  inner_join(., hs.gsea$res[[1]] %>% select(ID, hs.NES = NES, hs.padj = p.adjust)) %>% 
  left_join(., biodom %>% select(Biodomain, Subdomain, ID = GO_ID), by = 'ID') %>% 
...
```{r}
gsea.vs.amp <- fad.enr %>% mutate(model = str_c('5xFAD, ',age)) %>% unnest(res) %>% 
  select(model, term = ID, mm.nes = NES, mm.padj = p.adjust) %>% 
  nest( data = -c(model) ) %>%
  mutate(
    data = map(
      data,
      ~ hs.gsea$res[[1]] %>% 
        select(term = ID, hs.nes = NES, hs.padj = p.adjust) %>% 
        full_join( ., .x, by = 'term', na_matches = 'never') %>%
        mutate(
...
```{r}
comp <- crossing(model = gsea.vs.amp$model, set = bd.term.list$set)
head(comp)
```
...
comp <- comp %>% 
  mutate(
    terms = map2(model, set, 
                 ~ gsea.vs.amp %>% filter(model == .x) %>% pull(data) %>% .[[1]] %>% 
                   filter( term %in% bd.term.list$terms[[which(bd.term.list$set == .y)]] ) ),
    n.terms = map_dbl(terms, ~nrow(.x)),
    n.sig.both = map_dbl(terms, ~ .x %>% filter(mm.padj <= 0.05 & hs.padj <= 0.05) %>% nrow()),
...
comp <- comp %>% 
  filter(n.terms > 3) %>% 
  mutate(
    cor = map(terms, ~ cor.test(.x[['hs.nes']], .x[['mm.nes']], method = 'kendall')),
...
tmp <- comp %>% 
  ...
  mutate(model = factor(model, c('5xFAD, 10m','5xFAD, 6m','5xFAD, 4m')))

ggplot(tmp, aes(set, model)) +
...
The largest overlap is with the `Immune Response` domain terms, which are positively correlated between 5xFAD and the Mayo cohort, and the correlation gets stronger with age. There are also positively correlated terms for `Lipid Metabolism`, `Structural Stabilization` and `Vasculature` domains. There is a negative correlation for some `Synapse` domain terms -- i.e. they are down in the Mayo cohort but up in 5xFAD transcriptomes. 
...
l <- comp %>% 
  filter(
    grepl('_',set)==T, 
     (p.value < 5e-2 | abs(correlation) > 0.2)) %>% 
  pull(set)
...
ggplot(tmp, aes(model,set1)) +
  facet_grid(rows = vars(abbr), scales='free', space='free')+
  geom_tile(colour = "black", fill = "white") +
  geom_point(aes(fill = correlation, size = jaccard ), color = 'black', shape = 21, stroke = .3) +
  geom_point(data = subset(tmp, p.value <= 5e-2), color="black", shape=0, size= 5, stroke = .8) +
  scale_size( "Jaccard index", range = c(.5, 4), limits = c(1e-90,NA)) +
...
This tells us that the positive correlations in `Immune Response` are primarily focused on cytokine production, leukocyte chemotaxis while the correlations in the `Structural Stabilization` domain are primarily related to cell-cell junction organization, and the late `Vasculature` correlations have to do with angiogenesis. 
